...
> * Implement file input/output, control statement, and custom functions in R
...
> 2. File/data input/output using **readr**
> 3. Data cleaning using **tidyr**
...
gse69360 %>%
    filter(Length <= 50)
# keep genes that statisfy both of the conditions above
gse69360 %>%
...
# keep only rows with chrY in Chr OR have a length less than or equal to 100
...
# drop all columns execpt Geneid and total expression column
# Then sort the table based on the total expression, with highest expressed genes on top

gse69360 %>% 
  mutate(TotalExp = rowSums(.[7:25]) / (25-7)) %>%
  select(Geneid,TotalExp) %>%
...
gene_counts <- gse69360 %>%
    select(-Chr, -Start, -End, -Strand, -Length) %>%          # select on the geneid and the columns with expression values
    rename(AA_Stomach = "AA_Stomach",                         # rename couple of columns (doesn't need to be done for this primer)
           OA_Stomach = OA_Stomach1) %>%
    mutate(OA_Stomach2 = NULL, OA_Stomach3 = NULL) %>%        # remove a couple of columns (I don't think needs to be done for this primer)
    mutate(Geneid = gsub("\\.[0-9]*$", "", Geneid))           # in GeneID column get rid of the period and everything after it

head(gene_counts,n=20)
...
Make a histogram plot hwere there is a histogram for each chomosome
...
The last thing we will look at is plotting gene with high varience in expression across the samples.
The first thing to do is to detrmine which gene have high varience
